...
def evaluate_calibration_clipped(scales, model, mq, arch, bit_weights, bit_act):
    ...
    if eval_count % 20 == 0:
        ...
        # save scales
        f_name = "scales_{}_W{}A{}.pkl".format(arch, bit_weights, bit_act)
        f = open(os.path.join(proj_root_dir, 'data', f_name), 'wb')
        pickle.dump(scales, f)
        f.close()
        print("Scales saved to {}".format(f_name))
...
def main(args, ml_logger):
    ...
    res = opt.minimize(
        lambda scales: evaluate_calibration_clipped(scales, inf_model, mq, args.arch, args.bit_weights, args.bit_act),
        init.cpu().numpy(),
    ...
    f = open(os.path.join(proj_root_dir, 'data', f_name), 'rb')
    scales = pickle.load(f)
    f.close()
    print("Scales loaded from {} {}".format(f_name, scales))
    '''
